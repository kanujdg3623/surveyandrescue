...
class Edrone():
	...
	def __init__(self):
		...
		# OUR COMMENT: sample time is not required as the loop in main is executed at a constant frequecy given by rospy.Rate()
	...
	#	8. Add error_sum
		...
		for i in range(3):
			...
			if self.error[i]*(self.prev_value[i]-self.setpoint[i])<0:
				self.Iterm[i]=0    # OUR COMMENT: when there is a transition of error from -ve to +ve  or vice versa then i term is equated to zero 
			
			self.out[i] = self.Pterm[i] * self.Kp[i] + self.Iterm[i] * self.Ki[i] + self.Dterm[i] * self.Kd[i]		

			self.prev_value[i]=self.drone_position[i]
		
		self.cmd.rcRoll=1490 - self.out[0]
		self.cmd.rcPitch=1490 + self.out[1]                # OUR COMMENT: default 1500 value is changed to pre-stabalize the drone
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		r.sleep()
		key = getKey()
		if key == '\x03':  # OUR COMMENT: when ctrl+c is pressed drone is landed and disarmed to avoid hovering and crashing
			break
	e_drone.land()
	e_drone.disarm()
	termios.tcsetattr(sys.stdin, termios.TCSADRAIN, settings)
